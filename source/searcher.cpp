...
struct refinement_list_compare {
    bool operator()(const pair<double, refinement_list *> &a, const pair<double, refinement_list *> &b) const {
...
void add_to_q(state_merger *merger) {
    ...
    if (best_solution != -1.0 && result >= best_solution) {
        cerr << "convergence > best q_size: " << Q.size() << endl;
        return;
    }
    ...
    if (refs->empty()) {
        cerr << "convergence: " << result << " q_size: " << Q.size() << endl;
...
void bestfirst(state_merger *merger) {
    ...
    if (SEARCH_SINKS) {
        int temp_merge_sinks = CURRENT_CONFIG.MERGE_SINKS;
        int temp_merge_sinks_core = CURRENT_CONFIG.MERGE_SINKS_WITH_CORE;
        CURRENT_CONFIG.MERGE_SINKS = 0;
        CURRENT_CONFIG.MERGE_SINKS_WITH_CORE = 0;
        greedy(merger, 0, false);
        best_solution = -1;
        CURRENT_CONFIG.MERGE_SINKS = temp_merge_sinks;
        CURRENT_CONFIG.MERGE_SINKS_WITH_CORE = temp_merge_sinks_core;
    }
