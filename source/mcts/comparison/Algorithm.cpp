...
std::unique_ptr<Algorithm> createAlgorithm(const std::string_view name, state_merger* merger, std::shared_ptr<QualityEvaluation> evaluator, std::shared_ptr<MCTSNodeFactory> factory) {
    const std::unordered_map<std::string, std::function<std::unique_ptr<Algorithm>(std::shared_ptr<QualityEvaluation>, std::shared_ptr<MCTSNodeFactory>)>> table = {
