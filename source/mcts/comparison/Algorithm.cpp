...
refinement_vector Algorithm::run(const std::shared_ptr<MCTSNode> &root,
...
std::shared_ptr<MCTSNode>
Algorithm::getChild(const std::shared_ptr<MCTSNode> &node,
                    const refinement *refinement) {
  if (node == nullptr)
    return nullptr;

  for (auto mcts_node : node->getChildren()) {
    if (ref_equal(mcts_node->getRefinement(), refinement))
      return mcts_node;
  }
  return nullptr;
}
...
createAlgorithm(const std::string_view name, state_merger *merger,
      ...
      table = {
          {"greedy",
           [&](std::shared_ptr<QualityEvaluation> eval,
             ...
             return std::make_unique<GreedyAlgorithm>(merger, std::move(eval),
                                                      std::move(f));
           }},
      };
