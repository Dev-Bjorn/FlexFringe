...
bool state_merger::early_stop_merge(apta_node *left, apta_node *right, int depth, bool &keep_merging) {
    keep_merging = true;
    if (CURRENT_CONFIG.KTAIL != -1 && depth > CURRENT_CONFIG.KTAIL) return true;
    if (CURRENT_CONFIG.KSTATE != -1 && (left->size < CURRENT_CONFIG.KSTATE || right->size < CURRENT_CONFIG.KSTATE)) return true;

    keep_merging = false;
    if (left->red && CURRENT_CONFIG.STAR_FREE && counting_path_occurs(left, right)) return true;
    if ((left->red && CURRENT_CONFIG.RED_FIXED) || CURRENT_CONFIG.ALL_FIXED) {
...
bool state_merger::merge(apta_node *left, apta_node *right, int depth, bool evaluate, bool perform, bool test) {
    ...
    if (perform) {
        ...
        if (CURRENT_CONFIG.MERGE_DATA) left->get_data()->update(right->get_data());
    }
    if (evaluate) {
    ...
    for (auto &it: right->guards) {
        ...
        if (left_guard == nullptr || left_guard->target == nullptr) {
            ...
            if (CURRENT_CONFIG.MERGE_DATA) {
                left_guard = left->guard(i, it.second);
                left_guard->data->update(right_guard->data);
            }
        } else {
            if (test) {
                if (evaluate && !eval->consistent(this, left_guard, right_guard)) return false;
            }
            if (CURRENT_CONFIG.MERGE_DATA) {
                left_guard = left->guard(i, it.second);
                left_guard->data->update(right_guard->data);
...
void state_merger::undo_merge(apta_node *left, apta_node *right) {
    ...
    for (auto it = right->guards.rbegin(); it != right->guards.rend(); ++it) {
        ...
        if (CURRENT_CONFIG.MERGE_DATA) left_guard->data->undo(right_guard->data);
    ...
    if (CURRENT_CONFIG.MERGE_DATA) left->get_data()->undo(right->get_data());
...
bool state_merger::split_single(apta_node* new_node, apta_node* old_node, tail* t, int depth, bool evaluate, bool perform, bool test){
    ...
    if (test) {
        ...
        if (CURRENT_CONFIG.KTAIL != -1 && depth > CURRENT_CONFIG.KTAIL) return true;
        if (CURRENT_CONFIG.KSTATE != -1 && (new_node->size < CURRENT_CONFIG.KSTATE && old_node->size < CURRENT_CONFIG.KSTATE)) return true;
    }
...
void state_merger::undo_split_single(apta_node *new_node, apta_node *old_node, tail *t) {
    ...
    old_guard->data->split_undo(new_guard->data);
...
bool state_merger::split(apta_node *new_node, apta_node *old_node, int depth, bool evaluate, bool perform, bool test) {
    ...
    */
    ...
    for (auto it = new_node->guards.begin(); it != new_node->guards.end(); ++it) {
        ...
        if (perform) {
            old_guard->data->split_update(new_guard->data);
        }
        ...
        if (new_child->size == old_child->size) {
            ...
            if (perform) {
                old_child->get_data()->split_update(new_child->get_data());
            }
            if (evaluate) {
...
void state_merger::undo_split(apta_node *new_node, apta_node *old_node) {
    for (auto it = new_node->guards.rbegin(); it != new_node->guards.rend(); ++it) {
        ...
        old_child->get_data()->split_undo(new_child->get_data());
...
bool state_merger::split_init(apta_node *red, tail *t, int attr, int depth, bool evaluate, bool perform, bool test) {
    ...
    if (perform) {
        blue->get_data()->split_update(new_child->get_data());
    }
...
void state_merger::undo_split_init(apta_node *red, tail *t, int attr) {
    ...
    if (new_guard->target == nullptr) {
        ...
        blue->size += new_child->size;
        blue->get_data()->split_undo(new_child->get_data());
...
void state_merger::depth_check_fill(apta_node* node, std::map<int,apta_node*>* depth_map, int depth, bool use_symbol){
    if(CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH != -1 && depth > CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH) return;
    ...
    depth_node->get_data()->update(node->get_data());
...
bool state_merger::depth_check_run(apta_node *left, apta_node *right, bool use_symbol) {
    ...
    if (CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH == -1) {
        reset_max_depth = true;
        CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH = right_depth_map->size();
    }
    depth_check_fill(left, left_depth_map, 0, use_symbol);

    eval->reset(this);
    for (int depth = 0; depth <= CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH; depth++) {
        ...
        if (!merge_result) {
            if (reset_max_depth) CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH = -1;
            return false;
        }
    }
    if (reset_max_depth) CURRENT_CONFIG.DEPTH_CHECK_MAX_DEPTH = -1;
...
bool state_merger::pre_consistent(apta_node *left, apta_node *right) {
    ...
    if (!CURRENT_CONFIG.MERGE_ROOT && left->source == nullptr) return false;
    if (!CURRENT_CONFIG.MERGE_SINKS && (left->is_sink() || right->is_sink())) return false;
    if (!CURRENT_CONFIG.MERGE_SINKS_WITH_CORE && ((left->is_red() && !left->is_sink()) && right->is_sink())) return false;
    if (CURRENT_CONFIG.MARKOVIAN_MODEL > -1 && !is_path_identical(left, right, CURRENT_CONFIG.MARKOVIAN_MODEL)) return false;
    if (CURRENT_CONFIG.IDENTICAL_KTAIL > -1 && !is_tree_identical(left, right, CURRENT_CONFIG.IDENTICAL_KTAIL)) return false;
    if (left->is_sink()) {
        bool sink_check = false;
        if (CURRENT_CONFIG.CONVERT_SINK_STATES && eval->sink_convert_consistency(this, left, right)) sink_check = true;
        if (CURRENT_CONFIG.MERGE_IDENTICAL_SINKS && is_tree_identical(left, right, -1)) sink_check = true;
        if ((CURRENT_CONFIG.CONVERT_SINK_STATES || CURRENT_CONFIG.MERGE_IDENTICAL_SINKS) && !sink_check) return false;
    }
    if (CURRENT_CONFIG.MERGE_LOCAL > 0 && merged_apta_distance(left, right, -1) > CURRENT_CONFIG.MERGE_LOCAL) {
        if (CURRENT_CONFIG.MERGE_LOCAL_COLLECTOR_COUNT == -1 || num_distinct_sources(left) < CURRENT_CONFIG.MERGE_LOCAL_COLLECTOR_COUNT) return false;
    }
    if (CURRENT_CONFIG.STAR_FREE && counting_path_occurs(left, right)) {
        return false;
    }
    return true;
}
...
refinement *state_merger::test_merge(apta_node *left, apta_node *right) {
    ...
    if (CURRENT_CONFIG.PERFORM_MERGE_CHECK) {
        if (CURRENT_CONFIG.MERGE_WHEN_TESTING) merge_result = merge(left, right);
    ...
    if (CURRENT_CONFIG.USE_LOWER_BOUND && score_result < CURRENT_CONFIG.LOWER_BOUND) merge_result = false;
    if ((merge_result && !eval->compute_consistency(this, left, right))) merge_result = false;
    if (CURRENT_CONFIG.PERFORM_MERGE_CHECK && CURRENT_CONFIG.MERGE_WHEN_TESTING) undo_merge(left, right);

    if (CURRENT_CONFIG.PERFORM_DEPTH_CHECK && merge_result) {
        merge_result = depth_check_run(left, right, false);
    }
    if (CURRENT_CONFIG.PERFORM_SYMBOL_CHECK && merge_result) {
...
struct refinement_set_result {
    using collection_type = refinement_set *;

    static refinement_set *get() {
        return new refinement_set();
    }

    static void clear(refinement_set *refinements) {
        for (auto &it: *refinements) {
            it->erase();
        }
        refinements->clear();
    }
...
static_assert(CollectionType<refinement_set_result>);
...
refinement_set *state_merger::get_possible_refinements() {
    return get_possible_refinements(refinement_set_result{});
}
...
static_assert(CollectionType<refinement_tuple_result>);

std::tuple<refinement_vector, refinement_vector> state_merger::get_refinements() {
    return get_possible_refinements(refinement_tuple_result{});
}

template<typename Col> requires CollectionType<Col>
Col::collection_type state_merger::get_possible_refinements(Col collections) {
    auto collection = collections.get();
    ...
    if (!found_non_sink && !CURRENT_CONFIG.MERGE_SINKS) {
        return collection;
    }

    if (CURRENT_CONFIG.DFA_SIZE_BOUND != -1 && get_num_red_states() >= CURRENT_CONFIG.DFA_SIZE_BOUND) {
        return collection;
    }
    if (CURRENT_CONFIG.APTA_SIZE_BOUND != -1 && get_final_apta_size() <= CURRENT_CONFIG.APTA_SIZE_BOUND) {
        return collection;
    }
    ...
    for(auto it = blue_its.begin(); it != blue_its.end(); ++it){
        ...
        if (!blue->is_sink()) {
            if (dat->get_num_attributes() > 0) {
                ...
                if (ref != nullptr) {
                    collections.insert(collection, ref);
                    found = true;
                }
            }
        }
        ...
        if (CURRENT_CONFIG.MERGE_BLUE_BLUE) {
        ...
        if (CURRENT_CONFIG.EXTEND_ANY_RED && !found && !blue->is_sink()) {
            collections.clear(collection);
            collections.insertExtends(collection, mem_store::create_extend_refinement(this, blue));
            return collection;
        }

        if (!found || CURRENT_CONFIG.EXTEND_SINKS || !blue->is_sink()) {
            collections.insertExtends(collection, mem_store::create_extend_refinement(this, blue));
        }

        if (CURRENT_CONFIG.MERGE_MOST_VISITED) break;
    }
    return collection;
}
...
void state_merger::todot() {
    ...
    dot_output = "// produced with flexfringe // " + COMMAND_LINE + '\n' + dot_output_buf.str();
}
...
int state_merger::sink_type(apta_node *node) {
    if (CURRENT_CONFIG.USE_SINKS)
        return node->get_data()->sink_type();
    return -1;
}

bool state_merger::sink_consistent(apta_node *node, int type) {
    return node->get_data()->sink_consistent(type);
}

int state_merger::num_sink_types() {
    return aut->root->get_data()->num_sink_types();
