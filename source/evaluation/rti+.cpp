...
void rtiplus_data::add_tail(tail *t) {
    ...
    for (int i = 0; i < inputdata_locator::get()->get_num_attributes(); ++i) {
        ...
        if (!found) {
            quantile_counts[attr][rtiplus::attribute_quantiles[attr].size()] =
                    quantile_counts[attr][rtiplus::attribute_quantiles[attr].size()] + 1;
        }
    }
};
...
void rtiplus_data::set_loglikelihood() {
    ...
    for (num_map::iterator it = counts_begin(); it != counts_end(); ++it) {
        ...
        if (count >= CURRENT_CONFIG.SYMBOL_COUNT) divider += count + CURRENT_CONFIG.CORRECTION;
        else pool += count;
    }
    ...
    if (pool > 0.0) divider += pool + CURRENT_CONFIG.CORRECTION;

    for (num_map::iterator it = counts_begin(); it != counts_end(); it++) {
        double count = it->second;
        if (count < CURRENT_CONFIG.SYMBOL_COUNT);
        else if (count > 0) loglikelihood += (count + CURRENT_CONFIG.CORRECTION) * log(
                                (count + CURRENT_CONFIG.CORRECTION) / divider);
    }
    ...
    if (pool > 0) loglikelihood += (pool + CURRENT_CONFIG.CORRECTION) * log(
                      (pool + CURRENT_CONFIG.CORRECTION) / divider);
    ...
    if (pool > 0) divider += pool + CURRENT_CONFIG.CORRECTION;

    for (int i = 0; i < quantile_counts.size(); ++i) {
        for (int j = 0; j < rtiplus::attribute_quantiles[i].size() + 1; ++j) {
            double count = quantile_counts[i][j];
            if (count >= CURRENT_CONFIG.SYMBOL_COUNT) loglikelihood += (count + CURRENT_CONFIG.CORRECTION) * log(
                                                          (count + CURRENT_CONFIG.CORRECTION) / divider);
        }
    }
    if (pool > 0) loglikelihood += (pool + CURRENT_CONFIG.CORRECTION) * log(
...
void rtiplus::update_score(state_merger *merger, apta_node *left, apta_node *right) {
    ...
    if (CURRENT_CONFIG.FINAL_PROBABILITIES) {
        if (r->num_paths() + r->num_final() < CURRENT_CONFIG.STATE_COUNT || l->num_paths() + l->num_final() <
            CURRENT_CONFIG.STATE_COUNT) return;
    } else {
    ...
    for (int i = 0; i < l->quantile_counts.size(); ++i) {
        ...
        if (left_divider < CURRENT_CONFIG.STATE_COUNT || right_divider < CURRENT_CONFIG.STATE_COUNT) continue;
...
bool rtiplus::split_compute_consistency(state_merger *, apta_node *left, apta_node *right) {
    ...
    if (CURRENT_CONFIG.USE_SINKS && (left->get_size() <= CURRENT_CONFIG.SINK_COUNT || right->get_size() <=
                                     CURRENT_CONFIG.SINK_COUNT)) return false;
